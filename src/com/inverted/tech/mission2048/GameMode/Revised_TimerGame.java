...
public class Revised_TimerGame extends View implements Runnable {
	...
	private int totalTime = GameDisplay.timeRushLimit;
	...
	public Revised_TimerGame(final Context context) {
		super(context);
		Revised_TimerGame.mContext = context;

		DatabaseHandler db = new DatabaseHandler(context);
		db.setGameDataPoints(100000);
	...
	private void startGame() {
		...
		totalTime = GameDisplay.timeRushLimit;
	...
	protected void onDraw(Canvas canvas) {
		...
		// if (!threadSyncFlag)
		{
			...
			canvas.drawBitmap(bitmapTimeScoreHeader, scoreDisplay.x,
					scoreDisplay.y, null);
			canvas.drawBitmap(bitmapTimeScoreHeader, timeDisplay.x,
					timeDisplay.y, null);
			...
			canvas.drawBitmap(nightModeBitmap, nightModePoint.x,
					nightModePoint.y, null);
			...
			canvas.drawBitmap(footerMenuBitmap, autoMovePoint.x,
					autoMovePoint.y, null);
			canvas.drawBitmap(footerMenuBitmap, boostersPoint.x,
					boostersPoint.y, null);
			...
			pnt = getTextBound("AUTO MOVE", autoMoveTextPoint,
					footerMenuTextPaint);
			canvas.drawText("AUTO MOVE", pnt.x, pnt.y, footerMenuTextPaint);

			// time rush footer menu display
			pnt = getTextBound("POWERUPS", boostersTextPoint,
					footerMenuTextPaint);
